module Jekyll
  ...
  class Renderer
    alias_method :run_without_paginator, :run
    def run
      if document.respond_to? :data_pager
        payload["data_paginator"] = document.data_pager.to_liquid
      end
      run_without_paginator
    end
  end
  ...
  module DataPaginate
    class DataPager
      ...
      def self.pagination_candidate?(config, page)
        ...
        paginate_path = remove_leading_slash(config['data_paginate']['path'])
      ...
      def self.paginate_path(site, num_page)
        ...
        format = site.config['data_paginate']['path']
      ...
      def initialize(site, page, all_posts, num_pages = nil)
        @page = page
        @per_page = (site.config['data_paginate']['per_page'] || 5).to_i
